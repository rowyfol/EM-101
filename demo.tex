...
\begin{abstract}
\noindent
This report presents the design, theoretical analysis, and computational simulation of a single-bit memory cell based purely on electromagnetic principles. Unlike conventional transistor-based DRAM, this cell stores binary information as an electrostatic energy state within a parallel-plate capacitor. The project explores the physical foundations of charge storage, dielectric relaxation, and the finite conductivity of real materials that leads to data degradation over time. We derive the governing equations from Maxwell's electrostatics, implement a numerical solver using the Finite Difference Method (FDM) for potential distribution, and develop a Python-based transient simulator to model the complete memory cycle: Write, Hold, Read, and Refresh. The analysis demonstrates how the material relaxation time constant $\tau_r = \epsilon/\sigma$ serves as the fundamental engineering constraint determining the refresh rate and reliability of the stored bit.
\end{abstract}
